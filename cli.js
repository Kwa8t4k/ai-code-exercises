...
program
  ...
  program
    .command('export-csv <file>')
    .description('Export all tasks to CSV file')
    .action((file) => {
      const ok = taskManager.exportTasksCsv(file);
      if (ok) console.log(`Exported tasks to ${file}`);
      else console.log('Failed to export tasks to CSV.');
    });

  program
    .command('sweep-abandoned')
    .description('Mark tasks overdue > N days as ABANDONED (unless HIGH priority)')
    .option('-d, --days <n>', 'Days threshold', '7')
    .action((options) => {
      const days = parseInt(options.days || '7');
      const changed = taskManager.sweepAbandoned(days);
      console.log(`Sweep complete. Tasks updated: ${changed}`);
    });
