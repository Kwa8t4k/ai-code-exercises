...
class TaskManager {
  ...
  exportTasksCsv(filePath) {
    return this.storage.exportCSV(filePath);
  }

  sweepAbandoned(days = 7) {
    const tasks = this.storage.getAllTasks();
    const now = new Date();
    let changed = 0;
    ...
    tasks.forEach(task => {
      ...
      if (task.status === TaskStatus.DONE || task.status === TaskStatus.ABANDONED) return;

      const overdue = task.isOverdue();
      if (!overdue) return;

      const diffMs = now - task.dueDate;
      const diffDays = Math.floor(diffMs / (1000 * 60 * 60 * 24));

      if (diffDays > days && task.priority !== TaskPriority.HIGH) {
    ...
    if (changed > 0) this.storage.save();
    return changed;
  }
}
